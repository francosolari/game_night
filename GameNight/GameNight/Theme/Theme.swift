...
struct Theme {
    ...
    struct Colors {
        ...
        static var gameHighlight: Color {
            Color(UIColor { traitCollection in
                if traitCollection.userInterfaceStyle == .dark {
                    return UIColor(hex: BrandGuide.Dark.textPrimary).withAlphaComponent(0.12)
                } else {
                    return UIColor(hex: BrandGuide.Warm.espresso).withAlphaComponent(0.08)
                }
            })
        }
