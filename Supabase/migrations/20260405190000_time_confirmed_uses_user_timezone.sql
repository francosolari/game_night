-- Ensure host-confirmed date notifications use recipient timezone data.
...
begin
    ...
    select
        ...
        nullif(btrim(label), '')
    into v_start_time_ts, v_start_time, v_notification_time_text, v_time_label
    from time_options
    where id = p_time_option_id;
    ...
    select
        ...
        'time_confirmed',
        'Date Confirmed',
        coalesce(v_event.title, 'Game Night') || ' is locked in for ' || coalesce(
            v_time_label,
            to_char(v_start_time_ts at time zone coalesce(tz.name, 'UTC'), 'Dy, Mon FMDD at FMHH12:MI AM'),
            v_notification_time_text,
            v_start_time
        ) || '.',
        p_event_id,
        i.id,
        jsonb_build_object(
            ...
            'time_zone_identifier', coalesce(tz.name, 'UTC'),
            'time_display', coalesce(
                v_time_label,
                to_char(v_start_time_ts at time zone coalesce(tz.name, 'UTC'), 'Dy, Mon FMDD at FMHH12:MI AM'),
                v_notification_time_text,
                v_start_time
