...
-- 1. Update notification text to say "invite" not "added"
-- 2. Create DMs on group invite (like event invites)
-- 3. Auto-expire pending group invites after 7 days

-- ============================================================
-- 1. FIX NOTIFICATION TEXT: "invite" wording, not "added"
-- ============================================================

-- Update the insert-time trigger (fires when user_id is known at insert)
...
-- Update the signup-time trigger (fires when user_id is backfilled)
...
-- ============================================================
-- 2. DM ON GROUP INVITE (like event invite DMs)
-- ============================================================
...
declare
    ...
    v_inviter_name text;
begin
    -- Only for pending invites with a known inviter
    ...
    if new.user_id is not null then
        -- App user: create/find DM conversation and send message
...
drop trigger if exists trg_create_group_invite_dm on group_members;
create trigger trg_create_group_invite_dm
    after insert on group_members
    for each row
    execute function create_group_invite_dm();

revoke execute on function create_group_invite_dm() from public, anon, authenticated;

-- ============================================================
-- 3. AUTO-EXPIRE: decline pending group invites older than 7 days
-- ============================================================

create or replace function expire_stale_group_invites()
returns void
language plpgsql
security definer
set search_path = public
...
-- Add group_invite to message_type constraint
...
alter table direct_messages add constraint direct_messages_message_type_check
    check (message_type in ('text', 'invite', 'system', 'group_invite'));
